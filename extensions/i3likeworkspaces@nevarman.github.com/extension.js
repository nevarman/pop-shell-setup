...
/* Almost all of the credit belong to the original fmuellner's Workspace Indicator official extension */
/* https://gitlab.gnome.org/GNOME/gnome-shell-extensions/-/tree/master/extensions/workspace-indicator */
...
let WorkspaceThumbnail = GObject.registerClass({
    ...
    _init(index) {
        ...
        let wins = this._workspace.list_windows();
        wins.forEach(w => this._addWindow(w));
        this._onRestacked();
    }
...
let WorkspaceIndicator = GObject.registerClass(
    class WorkspaceIndicator extends PanelMenu.Button {
        ...
        _updateActiveThumbnail() {
            ...
            this._updateThumbnailByWorkspace();
        ...
        _updateThumbnails() {
            ...
            for (let i = 0; i < workspaceManager.n_workspaces; i++) {
                ...
                thumb.add_actor(statusLabel);
            }
            this._updateActiveThumbnail();
        }
        _updateThumbnailByWorkspace() {
            let thumbs = this._thumbnailsBox.get_children();
            let workspaceManager = global.workspace_manager;
            for (let i = 0; i < thumbs.length; i++) {
                let _workspace = workspaceManager.get_workspace_by_index(i);
                let wins = _workspace.list_windows().length;
                if (i == this._currentWorkspace || wins > 0)
                    thumbs[i].remove_style_class_name('inactive')
                else
                    thumbs[i].add_style_class_name('inactive');
            }
        }
        _activate(index) {
            let workspaceManager = global.workspace_manager;
